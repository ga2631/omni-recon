...
impl ShopeeParser {
    pub fn parse_income_statement<P: AsRef<Path>>(file_path: P) -> Result<Vec<StandardOrderRecord>> {
        let path = file_path.as_ref();
        info!("Parsing Shopee income statement: {:?}", path);
        ...
        let first_sheet = sheet_names.first()
            .ok_or_else(|| OmniError::Parser("No sheet found in Shopee workbook".to_string()))?;

        let range = workbook.worksheet_range(first_sheet)
            .map_err(|e| OmniError::Parser(format!("Failed to open sheet: {}", e)))?;

        let mut records = Vec::new();
        // Parse rows and map columns to StandardOrderRecord
        // Note: Shopee column headers vary slightly across versions, handled via heuristic mapping
        for row in range.rows().skip(1) {
            if let Some(order_id_cell) = row.get(0) {
                ...
                // Default empty/zero placeholder for unmapped numerical fields
                records.push(StandardOrderRecord {
                    order_id,
                    channel_code: "SHOPEE".to_string(),
        ...
        info!("Successfully parsed {} Shopee order records", records.len());
        Ok(records)
    }
}
