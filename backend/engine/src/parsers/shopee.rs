...
impl ShopeeParser {
    ...
    fn parse_range(range: &Range<Data>) -> Result<Vec<StandardSettlementRecord>> {
        ...
        let idx_gross = get_idx(&["giá bán gốc", "tổng giá bán", "doanh thu", "order amount", "giá niêm yết"]);
        ...
        let idx_service = get_idx(&["phí dịch vụ", "service fee", "phí freeship xtra", "phí hoàn xu"]);
        let idx_payment = get_idx(&["phí thanh toán", "payment fee", "phí xử lý giao dịch"]);
        let idx_buyer_ship = get_idx(&["phí vận chuyển người mua trả", "buyer shipping"]);
        let idx_seller_ship = get_idx(&["phí vận chuyển người bán trả", "phí vận chuyển", "shipping fee"]);
        let idx_subsidy_ship = get_idx(&["trợ giá vận chuyển từ shopee", "trợ giá vận chuyển", "shipping rebate"]);
        let idx_net = get_idx(&["số tiền nhận được", "tiền thực nhận", "net amount", "tổng tiền thanh toán", "actual amount"]);
        let idx_settled = get_idx(&["thời gian thanh toán", "ngày thanh toán", "settlement date", "thời gian chuyển tiền"]);
        ...
        for row in rows.iter().skip(header_row_idx + 1) {
            ...
            // Capture unmapped metadata into dynamic attributes
    ...
    fn parse_csv(path: &Path) -> Result<Vec<StandardSettlementRecord>> {
        ...
        let idx_payout_id = get_idx(&["mã phiên thanh toán", "mã đợt thanh toán", "payout id", "mã giao dịch"]);
        let idx_order_status = get_idx(&["trạng thái đơn hàng", "trạng thái", "status"]);
        let idx_buyer = get_idx(&["tên người mua", "người mua", "buyer"]);
        let idx_tracking = get_idx(&["mã vận đơn", "mã đvvc", "tracking"]);
        let idx_gross = get_idx(&["giá bán gốc", "tổng giá bán", "doanh thu", "order amount"]);
        let idx_seller_discount = get_idx(&["giảm giá của shop", "khuyến mãi của shop", "seller discount"]);
        let idx_voucher = get_idx(&["voucher của shopee", "trợ giá của shopee", "shopee voucher"]);
        let idx_comm = get_idx(&["phí cố định", "commission fee", "phí hoa hồng"]);
        let idx_service = get_idx(&["phí dịch vụ", "service fee", "phí freeship"]);
        let idx_payment = get_idx(&["phí thanh toán", "payment fee"]);
        let idx_buyer_ship = get_idx(&["phí vận chuyển người mua trả", "buyer shipping"]);
        let idx_seller_ship = get_idx(&["phí vận chuyển người bán trả", "phí vận chuyển", "shipping fee"]);
        let idx_subsidy_ship = get_idx(&["trợ giá vận chuyển từ shopee", "trợ giá vận chuyển"]);
        let idx_net = get_idx(&["số tiền nhận được", "tiền thực nhận", "net amount", "tổng tiền thanh toán"]);
        let idx_settled = get_idx(&["thời gian thanh toán", "ngày thanh toán", "settlement date"]);
